...
import { useForgotPasswordLink } from '../queries/forgot-password';
...
const Page = () => {
    ...
    const { mutate, isPending, isError, isSuccess, data, error } = useForgotPasswordLink();
            ...
            <div className="h-full flex flex-col p-4">
                ...
                <div className="flex-1 min-h-0 overflow-y-auto flex flex-col justify-center items-center">
                    <form className="flex flex-col  w-full gap-4 max-w-sm">
                        <h1 className="text-4xl text-[#2f1107] font-semibold md:text-5xl lg:text-6xl text-center mb-4">Forgot Password</h1>
                        ...
                        <div className="flex-1 flex flex-col gap-4 justify-center items-center">
                            <button disabled={isPending} onClick={() => mutate({ email })} className="inline-flex items-center justify-center gap-2 whitespace-nowrap cursor-pointer text-sm md:text-base font-medium transition-all bg-[#FFD100] text-[#2f1107] hover:bg-[#FFD100]/90 h-12 px-4 py-2 rounded-full w-full" type="button">
